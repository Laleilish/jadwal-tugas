...
export default function TaskList({ tasks, onTaskDeleted, filter }: TaskListProps) {
  ...
  const getFilteredTasks = () => {
    ...
    return tasks.filter(task => {
      ...
      if (filter === 'thisMonth') {
        return taskDeadline.getMonth() === currentMonth && taskDeadline.getFullYear() === currentYear;
      }
      if (filter === 'nextMonth') {
  ...
  if (tasks.length === 0 && filter === 'all') {
    return <p className="text-center text-gray-500 dark:text-gray-400 py-8">Belum ada tugas. Silakan tambahkan tugas baru.</p>;
  }
  
  if (filteredTasks.length === 0) {
     return <p className="text-center text-gray-500 dark:text-gray-400 py-8">Tidak ada tugas untuk periode ini.</p>;
  }
              ...
              <tr key={task.id} className={rowClass}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center text-sm text-gray-700 dark:text-gray-300">
                    <CalendarDays size={16} className="mr-2 text-gray-400 dark:text-gray-500" />
